...
```ts
  ...
  7  |     const logoutPage=new LogoutPage(page)
  ...
  13 |     //await expect(page.locator('#nameofuser')).toHaveText('Welcome Abhirammmm', { timeout: 10000 });
  14 |     //await expect(page).toHaveURL("https://www.demoblaze.com/")
  15 |     await logoutPage.logoutFn()
> 16 |     await expect(page.locator('signin2')).toHaveText('Sign up')
     |                                           ^ Error: expect(locator).toHaveText(expected) failed
  17 | })
```